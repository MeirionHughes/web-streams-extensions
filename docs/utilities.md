...
```typescript
import { isReadableStream } from 'web-streams-extensions';

if (isReadableStream(obj)) {
  // obj is guaranteed to be ReadableStream
...
```typescript
import { isTransform } from 'web-streams-extensions';

if (isTransform(op)) {
  // op is a TransformStream, can use with pipeThrough
  stream.pipeThrough(op);
} else {
  // op is a function, use with pipe
  stream.pipe(op);
}
```
...
```typescript
import { sleep } from 'web-streams-extensions';
...
A simple event signaling mechanism for coordination between async operations.

```typescript
import { Signal } from 'web-streams-extensions';
...
// Trigger the signal from elsewhere
signal.fire();
```

### `Gate`

A reusable async gate that can be opened and closed multiple times.

```typescript
import { Gate } from 'web-streams-extensions';
...
An async queue that blocks when empty.

```typescript
import { BlockingQueue } from 'web-streams-extensions';

const queue = new BlockingQueue<number>();

// This will block until something is added
const item = await queue.dequeue();

// Add items from another context
queue.enqueue(42);
```