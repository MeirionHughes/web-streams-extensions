	...
	dispatchEvent(event) {
		...
		if (this['on'+event.type]) {
			try {
				...
				console.error('[NodeWorker] Error in on' + event.type + ' handler:', err);
			}
		}
		...
		list.forEach(handler => {
			try {
				...
				console.error('[NodeWorker] Error in addEventListener handler:', err);
			}
		});
	}
...
function workerThread() {
	...
	threads.parentPort.on('message', data => {
		...
		event.data = data;
		if (q == null) {
			// Only dispatch to event target (handles addEventListener)
			// Don't call onmessage directly to avoid duplicate processing
			self.dispatchEvent(event);
		} else {
			q.push(event);
		}
	...
	threads.parentPort.on('error', err => {
		console.log('[NodeWorker] parentPort error:', err);
		err.type = 'Error';
		self.dispatchEvent(err);
	});

	// Also listen for uncaught exceptions and unhandled rejections
	process.on('uncaughtException', (err) => {
		console.log('[NodeWorker] uncaughtException:', err);
	});
	
	process.on('unhandledRejection', (reason, promise) => {
		console.log('[NodeWorker] unhandledRejection:', reason);
	});

	class WorkerGlobalScope extends EventTarget {
		constructor() {
			super();
			// Ensure onmessage property is properly accessible
			this.onmessage = null;
		}
		postMessage(data, transferList) {
	...
	if (type === 'module') {
		...
		Promise.resolve().then(() => {
			flush();
			// Keep the worker alive to receive messages
			// Keep the event loop alive so worker doesn't exit
			const keepAlive = setInterval(() => {}, 1000);
			// Store interval so it can be cleared on worker termination
			global.__keepAliveInterval = keepAlive;
		});
	}
}
...
function evaluateDataUrl(url, name) {
	...
	const result = VM.runInThisContext(data, {
		filename: 'worker.<'+(name || 'data:')+'>'
	});
	console.log('[NodeWorker] VM.runInThisContext completed');
	return result;
}
