...
describe("catchError", () => {
  ...
  describe("Virtual Time", () => {
    describe("Basic Behavior", () => {
      it("should handle empty stream without errors", async () => {
        const scheduler = new VirtualTimeScheduler();
        await scheduler.run(async ({ cold, expectStream }) => {
          const stream = cold('|');
          const result = pipe(stream, catchError(() => cold('xyz|', { x: 1, y: 2, z: 3 })));
          expectStream(result).toBe('|');
        });
      });

      it("should catch error and switch to fallback stream", async () => {
        const scheduler = new VirtualTimeScheduler();
        await scheduler.run(async ({ cold, expectStream }) => {
          const stream = cold('ab#', { a: 1, b: 2 }, "test-error");
          const fallback = cold('xyz|', { x: 100, y: 200, z: 300 });
          const result = pipe(stream, catchError(() => fallback));
          expectStream(result).toBe('abxyz|', { a: 1, b: 2, x: 100, y: 200, z: 300 });
        });
      });

      it("should not interfere when no error occurs", async () => {
        const scheduler = new VirtualTimeScheduler();
        await scheduler.run(async ({ cold, expectStream }) => {
      ...
      it("should handle immediate error", async () => {
        const scheduler = new VirtualTimeScheduler();
        await scheduler.run(async ({ cold, expectStream }) => {
          const stream = cold('#', undefined, "immediate-error");
          const fallback = cold('abc|', { a: 1, b: 2, c: 3 });
          const result = pipe(stream, catchError(() => fallback));
          expectStream(result).toBe('abc|', { a: 1, b: 2, c: 3 });
        });
      });

      it("should handle error at end", async () => {
        const scheduler = new VirtualTimeScheduler();
        await scheduler.run(async ({ cold, expectStream }) => {
          const stream = cold('abc#', { a: 1, b: 2, c: 3 }, "end-error");
          const fallback = cold('---xy|', { x: 100, y: 200 }); // Start at tick 3 to match error timing
          const result = pipe(stream, catchError(() => fallback));
          expectStream(result).toBe('abc---xy|', { a: 1, b: 2, c: 3, x: 100, y: 200 });
        });
      });
    });

    describe("Fallback Stream Variations", () => {
      it("should handle empty fallback stream", async () => {
        const scheduler = new VirtualTimeScheduler();
        await scheduler.run(async ({ cold, expectStream }) => {
          const stream = cold('a#', { a: 1 }, "test-error");
          const fallback = cold('-|'); // Empty fallback
          const result = pipe(stream, catchError(() => fallback));
          expectStream(result).toBe('a-|', { a: 1 });
        });
      });

      it("should handle synchronous fallback values", async () => {
        const scheduler = new VirtualTimeScheduler();
        await scheduler.run(async ({ cold, expectStream }) => {
          const stream = cold('a#', { a: 1 }, "test-error");
          const fallback = cold('-(xy)|', { x: 100, y: 200 }); // Synchronous values at tick 1
          const result = pipe(stream, catchError(() => fallback));
          expectStream(result).toBe('a-(xy)|', { a: 1, x: 100, y: 200 });
        });
      });

      it("should handle long fallback stream", async () => {
        const scheduler = new VirtualTimeScheduler();
        await scheduler.run(async ({ cold, expectStream }) => {
          const stream = cold('a#', { a: 1 }, "test-error");
          const fallback = cold('-x-y-z-|', { x: 100, y: 200, z: 300 });
          const result = pipe(stream, catchError(() => fallback));
          expectStream(result).toBe('a-x-y-z-|', { a: 1, x: 100, y: 200, z: 300 });
        });
      });
    });

    describe("Error Timing and Propagation", () => {
      it("should handle multiple errors with different fallbacks", async () => {
        const scheduler = new VirtualTimeScheduler();
        await scheduler.run(async ({ cold, expectStream }) => {
          let errorCount = 0;
          const stream = cold('a#', { a: 1 }, "first-error");

          const result = pipe(stream, catchError((error) => {
            errorCount++;
          ...
          expectStream(result).toBe('a-x|', { a: 1, x: 100 });
        });
      });

      it("should preserve timing with delayed error", async () => {
        const scheduler = new VirtualTimeScheduler();
        await scheduler.run(async ({ cold, expectStream }) => {
          const stream = cold('a--b---#', { a: 1, b: 2 }, "delayed-error");
          const fallback = cold('-------xy|', { x: 100, y: 200 }); // Start at tick 7 to match error
          const result = pipe(stream, catchError(() => fallback));
          expectStream(result).toBe('a--b----------xy|', { a: 1, b: 2, x: 100, y: 200 });
        });
      });
    });

    describe("Edge Cases", () => {
      it("should handle fallback that also errors", async () => {
        const scheduler = new VirtualTimeScheduler();
        await scheduler.run(async ({ cold, expectStream }) => {
          const stream = cold('a#', { a: 1 }, "source-error");
          const fallback = cold('-#', undefined, "fallback-error");
          const result = pipe(stream, catchError(() => fallback));
          expectStream(result).toBe('a-#', { a: 1 }, "fallback-error");
        });
      });

      it("should handle selector that throws", async () => {
        const scheduler = new VirtualTimeScheduler();
        await scheduler.run(async ({ cold, expectStream }) => {
          const stream = cold('a#', { a: 1 }, "source-error");
          const selectorError = new Error("selector-error");
          const result = pipe(stream, catchError(() => {
            throw selectorError;
          }));
          expectStream(result).toBe('a#', { a: 1 }, selectorError);
        });
      });
    });
  });
